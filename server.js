...
app.delete("/deleteNote/:noteId", express.json(), async (req, res) => {
    try {
        ...
        jwt.verify(token, "secret-key", async (err, decoded) => {
            ...
            //.deleteOne returns 1 if it deleted something, and 0 if it didn't
            ...
            res.json({ response: "Note deleted successully" });
        });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
    ...
    try {
        ...
        jwt.verify(token, "secret-key", async (err, decoded) => {
            ...
            if (!data) {
                return res
                    .status(404)
                    .json({ error: "Unable to find note with given ID. Occurs at .data" });
            }
           ...
           if ((title && title.length > 0) && (content && content.length > 0)){
            ...
            updatedInfo.content = content;
           } else if ((title && title.length > 0)){
            updatedInfo.title = title;
            updatedInfo.content = data.content;
           } else if ((content && content.length > 0)){
            updatedInfo.title = data.title;
            updatedInfo.content = content;
           } else {
            updatedInfo.title = data.title;
            updatedInfo.content = data.content;
           ...
           const doUpdate = await collection.updateOne(filter, {$set: updatedInfo});

           if (doUpdate.modifiedCount != 1){
                return res
                    .status(404)
                    .json({ error: "Unable to find note with given ID." });
           }
